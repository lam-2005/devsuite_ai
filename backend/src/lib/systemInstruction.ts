...
4. ai_status: Set to "completed" if analysis succeeds. If the input is completely unreadable, corrupted, or missing critical data, set to "failed".
...
# FAILURE HANDLING
If you cannot analyze the log due to severe data corruption, return a JSON object where 'ai_status' is 'failed', and 'ai_reason' and 'ai_suggestion' explain the failure in the requested [TARGET_LANGUAGE].
`;
